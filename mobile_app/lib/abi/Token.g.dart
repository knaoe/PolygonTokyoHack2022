...
class Token extends _i1.GeneratedContract {
  ...
  Future<String> AirDrop(
      List<_i1.EthereumAddress> _targetAddresses, List<BigInt> _value,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[1];
    assert(checkSignature(function, '51524e5b'));
    final params = [_targetAddresses, _value];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i2.Uint8List> SNAPSHOT_ROLE({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[4];
    assert(checkSignature(function, '7028e2cd'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as _i2.Uint8List);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> balanceOfAt(_i1.EthereumAddress account, BigInt snapshotId,
      {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[8];
    assert(checkSignature(function, '4ee2cd7e'));
    final params = [account, snapshotId];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> totalSupplyAt(BigInt snapshotId,
      {_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[25];
    assert(checkSignature(function, '981b24d0'));
    final params = [snapshotId];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// Returns a live stream of all Snapshot events emitted by this contract.
  Stream<Snapshot> snapshotEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('Snapshot');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
...
class Snapshot {
  Snapshot(List<dynamic> response) : id = (response[0] as BigInt);
